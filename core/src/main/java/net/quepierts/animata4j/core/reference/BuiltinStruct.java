package net.quepierts.animata4j.core.reference;

import com.google.common.collect.ImmutableList;
import net.quepierts.animata4j.core.data.layout.*;
...
public class BuiltinStruct {
    ...
    static {
        ...
        VEC2 = StructDefinition.builder("vec2")
                .field(FieldDefinition.primitive(AnimataDataType.FLOAT).name("x").build())
                .field(FieldDefinition.primitive(AnimataDataType.FLOAT).name("y").build())
                .build();
        builder.add(VEC2);
        ...
        VEC3 = StructDefinition.builder("vec3")
                .field(FieldDefinition.primitive(AnimataDataType.FLOAT).name("x").build())
                .field(FieldDefinition.primitive(AnimataDataType.FLOAT).name("y").build())
                .field(FieldDefinition.primitive(AnimataDataType.FLOAT).name("z").build())
                .build();
        builder.add(VEC3);
        ...
        VEC4 = StructDefinition.builder("vec4")
                ...
                .field(FieldDefinition.primitive(AnimataDataType.FLOAT).name("y").build())
                .field(FieldDefinition.primitive(AnimataDataType.FLOAT).name("z").build())
                .field(FieldDefinition.primitive(AnimataDataType.FLOAT).name("w").build())
                .build();
        builder.add(VEC4);
